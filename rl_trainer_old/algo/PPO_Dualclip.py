...
class Actor(nn.Module):
    def __init__(self):  #(n+2p-f)/s + 1 
        ...
        self.linear_1 = nn.Linear(768, 12)
...
class Critic(nn.Module):
    def __init__(self):
        ...
        self.conv3_1 = nn.Conv2d(8, 8, kernel_size=3, stride=1, padding=0) # 18816 -> 16632
        #self.conv4_1 = nn.Conv2d(16, 32, kernel_size=3, stride=1, padding=0) # 16632 -> 14464
        self.linear_CNN_1_1 = nn.Linear(768, 3) #14464 = 3584
        ...
        self.conv3_2 = nn.Conv2d(8, 8, kernel_size=3, stride=1, padding=0) # 18816 -> 16632
        #self.conv4_2 = nn.Conv2d(16,32, kernel_size=3, stride=1, padding=0) # 16632 -> 14464
        self.linear_CNN_1_2 = nn.Linear(768, 3) #14464 = 3584
    ...
    def forward(self, tensor_cv):
        ...
        #
        x=  x.reshape(batch_size,1,768)#.repeat(1,3,1)
...
class PPO:
    ...
    def update(self,new_lr):
        ...
        for reward, is_terminal,values_1,values_2 in zip(reversed(self.memory.rewards), reversed(self.memory.is_terminals),
            ...
            discounted_advatage = reward + (1-is_terminal)*(discounted_advatage ) - values #np.maximum(values_1,values_2))#values
            GAE_advantage.insert(0, discounted_advatage) #插入列表
            discounted_advatage = self.gamma*self.gamma*discounted_advatage + self.gamma*values#np.minimum(values_1,values_2) #values #
        ...
        for _ in range(self.K_epochs):
            ...
            #
            actor_loss = -(surr3 + 50*self.a_lr*dist_entropy).mean()#.sum(-1)   100*self.a_lr 0.01*  .mean()
