...
class Actor(nn.Module):
    def __init__(self):  #(n+2p-f)/s + 1 
        ...
        self.conv1 = nn.Conv2d(4,4, kernel_size=3, stride=1, padding=1) # 20104 -> 20108
        self.conv2 = nn.Conv2d(4, 8, kernel_size=3, stride=1, padding=0) # 20108 -> 18816
        self.conv3 = nn.Conv2d(8, 16, kernel_size=3, stride=1, padding=0) # 18816 -> 16632
        self.conv4 = nn.Conv2d(16, 32, kernel_size=3, stride=1, padding=0) # 16632 -> 14464

        #self.linear_1 = nn.Linear(1792, 64) #14464 = 3584

        self.MU = nn.Linear(1792,3)
        self.STD = nn.Linear(1792,3)
    ...
    def forward(self, tensor_cv): #,batch_size
        ...
        #x = F.relu(self.linear_1(x)).reshape(x.size()[0],64)
...
class Critic(nn.Module):
    def __init__(self):
        ...
        self.linear_CNN_1_1 = nn.Linear(1792, 3) #14464 = 3584  896
    ...
    def forward(self, tensor_cv, h_state=(torch.zeros(1,4,128).to(device),
        ...
        x = x.reshape(x.size()[0],1,1792)
        #
        out = torch.tanh(self.linear_CNN_1_1(x)).reshape(x.size()[0],3)
...
class PPO:
    ...
    def update(self,new_lr):
        ...
        state_values = self.critic(old_states) #,_
        ...
        for _ in range(self.K_epochs):
            ...
            actor_loss = -(torch.min(surr1, surr2) + 20*self.a_lr*dist_entropy).mean() #100*self.a_lr
