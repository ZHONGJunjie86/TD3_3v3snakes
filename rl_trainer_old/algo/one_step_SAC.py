...
class Actor(nn.Module):
    def __init__(self):  #(n+2p-f)/s + 1 
        ...
        self.conv2 = nn.Conv2d(4, 8, kernel_size=3, stride=1, padding=0) # 20104 -> 20104
        self.conv3 = nn.Conv2d(8, 8, kernel_size=3, stride=1, padding=0) # 20104 -> 20104
...
class Critic(nn.Module):
    ...
    def forward(self, tensor_cv,action, h_state_1=(torch.zeros(1,1,32).to(device),torch.zeros(1,1,32).to(device)),
        ...
        x = F.relu(self.conv3_2(x)).reshape(1,4,192)
        #
