...
def train(shared_model,rank,args,device,log_dir,run_dir,shared_lock):
    #global shared_lock
    ...
    shared_lock.acquire()
    model.load_state_dict(shared_model.get_actor().state_dict()) # sync with shared model
    model_enemy.load_state_dict(shared_model.get_actor().state_dict())
    shared_lock.release()
    ...
    while episode < args.max_episodes:
        ...
        while True:
            ...
            if args.episode_length <= step: # or (True in done)
                ...
                shared_lock.acquire()
                for i in save_block:
                    shared_model.push(i[0],i[1],i[2],i[3],i[4])
                ...
                if rank == 0:
                    ...
                    if c_loss and a_loss and rank <= 3:
...
if __name__ == '__main__':  #
    ...
    parser.add_argument('--buffer_size', default=int(3e6), type=int) #1e5
    ...
    parser.add_argument("--load_model_run", default=10, type=int)
    ...
    #Multiprocessing
    parser.add_argument('--processes', default=14, type=int, help='number of processes to train with')
    ...
    for rank in range(args.processes):
        ...
        p = mp.Process(target=train, args=(model_share, rank, args,device,log_dir,run_dir,shared_lock)) #rank 编号
