...
from algo.one_step_SAC import Actor_Critic
from common import *
...
def main(args):
    ...
    while episode < args.max_episodes:
        ...
        first_time = True
        num_state = torch.DoubleTensor([0,0,0,0,0,0]).reshape(1,6).to(device)
        ...
        while True:
            ...
            logits = model.choose_action(obs,first_time) #num_state,
            first_time = False
            #print("logits",logits)
            ...
            num_state_next = torch.DoubleTensor(reward).reshape(1,6).to(device) #/10
            ...
            #model.update(new_lr,next_obs,num_state_next,step_reward,done)

            obs = next_obs
            num_state = num_state_next
            step += 1
...
#/home/j-zhong/work_place/TD3_SAC_PPO_multi_Python/one_step_SAC_history.png
if __name__ == '__main__':
    ...
    parser.add_argument('--tau', default=0.005, type=float) #005
    ...
    parser.add_argument("--save_interval", default=10, type=int) #1000
    parser.add_argument("--model_episode", default=0, type=int)
    parser.add_argument('--log_dir', default=datetime.datetime.now().strftime('%Y%m%d_%H%M%S'))

    parser.add_argument("--load_model", action='store_true')  # 加是true；不加为false
    parser.add_argument("--load_model_run", default=5, type=int)
